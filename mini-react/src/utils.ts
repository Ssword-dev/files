...
function isPrimitive(o: any): o is Primitive {
  const to = typeof o;
...
function defaultDiff(oldProps: any, newProps: any): ReconcilerAttributeChange {
  const changes: ReconcilerAttributeChange = {
    modifications: {},
    additions: {},
    deletions: [],
  };
  ...
  for (const ok in oldProps) {
    if (
      typeof changes.additions[ok] === "undefined" &&
